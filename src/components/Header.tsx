...
export default function Header() {
  ...
  const navLinks = [
    ...
    { label: t('common:menu.skinCheck'), to: `/${lang}/skin-check` },
            ...
            <nav className="hidden items-center gap-1 lg:flex">
              ...
              {navLinks.slice(1).map((link) => (
                <Link
                  key={link.label}
                  to={link.to}
                  className="flex items-center gap-1.5 rounded-full px-4 py-2 text-sm font-medium text-charcoal-700 transition-colors hover:bg-rose-50 hover:text-rose-600"
                >
                  {link.to.endsWith('/skin-check') ? <ScanFace className="h-4 w-4" strokeWidth={1.8} /> : null}
                  {link.label}
                </Link>
              ))}
            </nav>
