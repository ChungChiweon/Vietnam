...
export default function SkinAnalysisPage() {
  ...
  const recommendationCopy = {
    ko: { title: '추천 상품', priorities: '우선 관리 방향', empty: '현재 등록된 상품 중 검증된 데이터가 일치하는 상품이 없습니다. 추천 성분과 관리 방향을 참고해 주세요.', retry: '정확한 상품 추천을 위해 촬영 조건을 개선하여 다시 분석해 주세요.', notice: '촬영 이미지와 설문을 바탕으로 한 화장품 추천용 피부 상태 추정입니다.', need: '관리 필요도와 일치', benefit: '관리 방향 태그', ingredient: '검증 성분 포함', skin_type: '피부 타입 태그' },
    vi: { title: 'Sản phẩm đề xuất', priorities: 'Hướng chăm sóc ưu tiên', empty: 'Hiện chưa có sản phẩm nào có dữ liệu đã xác minh phù hợp. Hãy tham khảo thành phần và hướng chăm sóc.', retry: 'Vui lòng chụp lại trong điều kiện tốt hơn để nhận gợi ý đáng tin cậy.', notice: 'Đây là ước tính phục vụ lựa chọn mỹ phẩm dựa trên ảnh và bảng hỏi.', need: 'Phù hợp nhu cầu chăm sóc', benefit: 'Lợi ích phù hợp', ingredient: 'Có thành phần đã xác minh', skin_type: 'Phù hợp loại da' },
    en: { title: 'Recommended Products', priorities: 'Priority care directions', empty: 'No products currently have verified matching data. Use the suggested ingredients and care direction instead.', retry: 'Retake the photo in better conditions for reliable product matching.', notice: 'This is a cosmetic recommendation estimate based on the photo and questionnaire.', need: 'Matches a care need', benefit: 'Matching benefit tag', ingredient: 'Contains a verified ingredient', skin_type: 'Matching skin type tag' },
  }[lang];
              ...
              {step === 0 ? (
                ...
                <div className="animate-fade-in">
                  ...
                  <div className="mt-6 rounded-2xl border border-rose-100 bg-white p-5 sm:p-6">
                    ...
                    <p className="mt-2 text-xs leading-5 text-charcoal-700/45">{recommendationCopy.notice}</p>
                    {recommendations.needs.length ? <div className="mt-4"><p className="text-xs font-semibold uppercase tracking-wider text-rose-500">{recommendationCopy.priorities}</p><div className="mt-2 flex flex-wrap gap-2">{recommendations.needs.map((need) => <span key={need} className="rounded-full bg-rose-50 px-3 py-1.5 text-xs text-rose-700">{need}</span>)}</div></div> : null}
                    {recommendations.products.length ? <div className="mt-4 grid gap-3 sm:grid-cols-3">{recommendations.products.map(({ product, recommendationReasons }) => {
                      const localized = getLocalizedProduct(product, lang);
                      return <Link key={product.id} to={`/${lang}/products/${product.slug}`} className="rounded-xl border border-rose-100 p-4 transition hover:border-rose-300 hover:shadow-sm"><p className="text-sm font-semibold">{localized.name}</p><ul className="mt-3 space-y-1.5">{recommendationReasons.slice(0, 3).map((reason) => <li key={`${reason.type}-${reason.key}`} className="text-[11px] leading-4 text-charcoal-700/55">• {recommendationCopy[reason.type]}: {reason.key}</li>)}</ul></Link>;
